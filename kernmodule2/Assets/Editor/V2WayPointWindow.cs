...
public class V2WayPointWindow : EditorWindow
{
    ...
    //private
    public V2WayPointSystem currentWayPointSystem = null;

    [MenuItem("Tools/V2 Waypoint Editor")]
    public static void Open()
    {
        GetWindow<V2WayPointWindow>();
    }
    ...
    private void OnGUI()
    {
        ...
        else
        {
            ...
            if (currentWPElement != null)
            {
                ...
                {
                    currentWayPointSystem.InsertAfter(currentWPElement.transform.GetComponent<V2WayPointObject>());
                }
                if (GUILayout.Button("Before selected"))
                {
                    currentWayPointSystem.InsertBefore(currentWPElement.transform.GetComponent<V2WayPointObject>());
                }
            }
        }
    }
    ...
    private void CreateNewWayPointSystem()
    {
        ...
        currentWayPointSystem = gm.AddComponent<V2WayPointSystem>();
        currentWayPointSystem.currentPrefab = activeObjectPrefab;
        currentWayPointSystem.Append();
    }
    ...
    private void UpdateSettings()
    {
        ...
        {
            currentWayPointSystem = currentSelected.GetComponent<V2WayPointSystem>();
            //geselecteerde object is geen waypointsysteem
            if (currentWayPointSystem == null)
            {
                currentWayPointSystem = currentSelected.transform.root.GetComponent<V2WayPointSystem>();
